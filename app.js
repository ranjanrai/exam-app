...
document.addEventListener('DOMContentLoaded', ()=> {
  ...
  // ✅ Hook up "Enable Camera" button
  const camBtn = document.getElementById('startCameraBtn');
  if (camBtn) camBtn.addEventListener('click', initHomeCamera);
});
...
function updateLiveCountBadge(sessionsArr) {
  try {
    ...
    const liveCount = (sessionsArr || []).filter(sess => {
      // adapt to the timestamp field your sessions docs use:
    ...
    if (!titleEl) {
      ...
      if (card) {
        ...
        if (!titleEl) {
          ...
          titleEl.textContent = 'Live Sessions — Who is giving exam?';
          card.insertBefore(titleEl, card.firstChild);
        }
      }
    }

    // set title text with count
    if (titleEl) {
      titleEl.textContent = `Live Sessions — Who is giving exam? (${liveCount} live)`;
      // optionally make the count visually stand out:
